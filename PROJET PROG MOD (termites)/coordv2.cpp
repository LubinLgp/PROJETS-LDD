...
Coord devantCoord(Direction dir, Coord c){
	...
	switch(dir){
        ...
        default : x=c.getLig();y=c.getCol();break;
	}
    if(x<0)x=TailleGrille+x;
    if(y<0)y=TailleGrille+y;
    x=x%TailleGrille;
    y=y%TailleGrille;
	if(x==5 and y==5){return Coord{15,15};}                                     //les deux conditions ici sont le moyen de creer des teleporter
	else if(x==15 and y==15){return Coord{5,5};}                                
    return Coord{x,y};
}
