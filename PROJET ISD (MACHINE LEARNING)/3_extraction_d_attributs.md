...
s le vôEn mettant en commentaire la ligne 2 puis la ligne 1, vous choisirez
...
images = load_images(dataset_dir, "*.png")
clean_images = images
clean_sample = list(clean_images[:15]) + list(clean_images[-15:])#Ce n'est pas parfait: du fait des groupes de pixels à droite qui ont
...
ecify a fill 'valuecify a fill 'valuecify a fill 'valuecify a fill 'valuOn constate que le recadrage sur le fruit est amélioré, même si pas
encore parfait.
...
Les zones claires correspondent aux endroits où il y a du rouge tandis que les endroits sombres sont les endroits ne contenant pas ou très peu de rouge.
...
ecify a fill 'valuecify a fill 'valuPour d'autres idées de mesures sur les couleurs, consulter [cette page
...
Le graphique nous indique que le bon nombre d'attributs est 4, ce qui peut parraitre contre-intuitif car on pourrait penser que plus on a de features plus le score devrait être bon.
...
La paire d'attribut donnant les meilleures performances est redness - R-G. Pour notre jeu de données, l'approche multi-variée n'est pas du tout nécéssaire car les résultats sont très corrects avec l'approche univariée
...
En travaillant sur cette feuille, nous avons compris que notre jeu de donnée n'était pas adapté pour être recentré automatiquement, nous l'avons donc fait manuellement. De plus, nous nous sommes rendus compte que le nombre d'attributs n'est pas directement corrélés à la performance du classificateur. En effet, pour notre jeu de donnée, un nombre d'attribut de 14 est visiblement plus adapté que 1 ou 23 par exemple.
