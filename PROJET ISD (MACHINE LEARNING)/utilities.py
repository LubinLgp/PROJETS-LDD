...
def blackness(img: Image.Image) -> float:
    M = np.array(img)
    compteur=0
    for ligne in M:
        for pixel in ligne:
            if pixel[0]==0 and pixel[1]==0 and pixel[2]==0:
                compteur+=1
    return compteur/(M.shape[0]*M.shape[1])

def espacemilieu(img: Image.Image) -> float:
    M=np.array(img)
    compteur=0
    for ligne in M:
        noir = False
        ...
        espace = False
        for pixel in ligne:
            if not noir2 and not noir and pixel[0]==0 and pixel[1]==0 and pixel[2]==0:
                noir = True
            if noir and pixel[0]==255 and pixel[1]==255 and pixel[2]==255:
                espace = True
            if noir and espace and pixel[0]==0 and pixel[1]==0 and pixel[2]==0:
                noir2=True
        if noir2:
            compteur+=1
    return compteur
